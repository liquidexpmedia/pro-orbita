// ==========================================
// ÓRBITA #1 - ZINE VIVO (Diseño Móvil - Scroll Horizontal V3)
// ==========================================
...
const HORIZONTAL_SPREAD_FACTOR = 3; // How much wider the scrollable area is than the viewport
...
const VERTICAL_JITTER = 15; // Max vertical random movement in pixels from the center line
...
function createAndPositionItems() {
  ...
  items.forEach((item, index) => {
    ...
    for (let attempt = 0; attempt < MAX_PLACEMENT_ATTEMPTS_PER_ITEM; attempt++) {
        ...
        // Calculate center vertical position
        const centerY = (window.innerHeight / 2) - (itemHeight / 2);
        // Add a random jitter around the center line
        const verticalJitter = (Math.random() - 0.5) * 2 * VERTICAL_JITTER;
        const topPos = centerY + verticalJitter;
    ...
    if (!placed) {
        ...
        el.style.top = `${centerY + (Math.random() - 0.5) * 2 * VERTICAL_JITTER}px`;
        el.style.visibility = 'visible';
    }
...
console.log('🚀 Zine script (mobile-friendly V3) cargado.');